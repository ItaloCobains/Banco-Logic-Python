...
def efetuar_saque() -> None:
    if len(contas) > 0:
        ...
        print('Ainda não existe contas cadrastradas.')
    sleep(2)
    menu()
...
def efetuar_deposito() -> None:
    if len(contas) > 0:
        ...
        if conta:
            ...
            print(f'Nao foi encontrada uma conta com nuemro {numero}')
    else:
        print('Ainda não existe contas cadrastrada')
    sleep(2)
    menu()
...
def efetuar_transferencia() -> None:
    if len(contas) > 0:
        ...
        if conta_o:
            ...
            if conta_d:
                valor: float = float(input("Informe um valor da transferencia: "))
                conta_o.transferir(conta_d, valor)

            else:
                print(f'A conta destino com o numero {numero_d} nao foi encontrada')
        else:
            print(f'A sua conta com o numero {numero_o} nao foi encontrada')
    else:
